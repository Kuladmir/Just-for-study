...
如果**使用绝对路径的方式调用gmx**，则可以使用下面绝对路径的命令执行；
或者也可以提前导入环境变量：
#可以使用绝对路径的方式，运行gmx，也可以提前导入一个环境变量：
```
export PATH=/pubhome/soft/gromacs/2023.2/threadMPI_OpenMP/bin:$PATH
```
运行完此命令后，则不需要再以环境变量的方式调用gmx命令了。之后就可以改为：
```
...
```
**
/pubhome/soft/gromacs/2020.5/thread_mpi/bin/gmx grompp -f em.mdp -r solv_ions.gro -c solv_ions.gro -p topol.top -o em.tpr -maxwarn 2 -n index.ndx
...
```
**
/pubhome/soft/gromacs/2020.5/thread_mpi/bin/gmx grompp -f nvt.mdp -c em.gro -r em.gro -p topol.top -o nvt.tpr -maxwarn 2 -n index.ndx
...
```
**
/pubhome/soft/gromacs/2020.5/thread_mpi/bin/-f npt.mdp -c nvt.gro -t nvt.cpt -r nvt.gro -p topol.top -o npt.tpr -maxwarn 2 -n index.ndx
...
```
**
/pubhome/soft/gromacs/2020.5/thread_mpi/bin/gmx grompp -f md.mdp -c npt.gro -t npt.cpt -p topol.top -n index.ndx -o md_0_10.tpr

/pubhome/soft/gromacs/2020.5/thread_mpi/bin/gmx mdrun -ntmpi 1 -deffnm md_0_10
```
### >7.2  运行任务提交脚本
由于gromacs运行需要在GPU集群上进行，因此需要对文件中的#SBATCH进行设置。同时，由于GPU计算需要Cuda等环境，因此需要先设置环境变量。
```
export LD_LIBRARY_PATH=/usr/local/cuda-11.1/lib64:$LD_LIBRARY_PATH
```
创建一个run_md.sh文件，在里面写入以下内容。
**注意：这个脚本是在ampere队列中使用，nodelist部分不一定非要是k229，应根据实际选择节点；function里，需要根据自己设置保存路径**
```
#!/bin/bash
...
function cleanup () {
    test -d $tmpdir && \cp -rf $tmpdir/$my_base $my_dir && rm  -rf $tmpdir
    test -d $my_dir/$my_base && scp -r $my_dir/$my_base usr@k143.hn.org:/pubhome/usr/test
    }
trap cleanup EXIT SIGUSR1 SIGUSR2

cwd=$(pwd)
my_base=$(basename $cwd)
my_dir=$(dirname $cwd)
tmpdir=$(mktemp -d -p /tmp)
echo "cwd: $cwd" > iOEA_0.out 2>&1
echo "tmpdir: $tmpdir" >> iOEA_0.out 2>&1
cp -rf $cwd/ $tmpdir
cd $tmpdir/$my_base

gmx grompp -f em.mdp -r solv_ions.gro -c solv_ions.gro -p topol.top -o em.tpr -maxwarn 2 -n index.ndx >> MD_S_conf1.out 2>&1
gmx mdrun -ntmpi 1 -v -deffnm em >> MD_S_conf1.out 2>&1
gmx grompp -f nvt.mdp -c em.gro -r em.gro -p topol.top -o nvt.tpr -maxwarn 2 -n index.ndx >> MD_S_conf1.out 2>&1
gmx mdrun -ntmpi 1 -deffnm nvt >> MD_S_conf1.out 2>&1
gmx grompp -f npt.mdp -c nvt.gro -t nvt.cpt -r nvt.gro -p topol.top -o npt.tpr -maxwarn 2 -n index.ndx >> MD_S_conf1.out 2>&1
gmx mdrun -ntmpi 1 -deffnm npt >> MD_S_conf1.out 2>&1
gmx grompp -f md.mdp -c npt.gro -t npt.cpt -p topol.top -o md_0_10.tpr -maxwarn 2 -n index.ndx >> MD_S_conf1.out 2>&1
gmx mdrun -ntmpi 1 -deffnm md_0_10 >> MD_S_conf1.out 2>&1
source /usr/bin/end_cuda.sh
```

**说明：自第五步开始后，所执行的一些命令（用\*\*标识的），可以直接使用上述脚本执行，无需再分步完成。但是第五步后的mdp文件的创建、索引的创建需要在运行脚本之前完成。**

然后输入以下命令（Slurm作业系统）：
```
sbatch run_md.sh
```
之后可以使用如下等命令，进行查看工作状态。
```
squeue

sacct
```
在本例中，如果有错误信息，会写入到MD_S_conf1.out文件里。
## 8.分析
<分析部分根据需求使用，仅供参考>
### 8.1  重定位和坐标重绘制
```
